...
function Signup({navigation}) {
    ...
    return (
        ...
        <TextInput onChange={phoneNumber=>setPhoneNumber(phoneNumber.nativeEvent.text)}/>
        <Text>Name</Text>
        <TextInput onChange={name=>setName(name.nativeEvent.text)}/>
        <Text>Password</Text>
        <TextInput onChange={password=>setPassword(password.nativeEvent.text)}/>
        <Button title={"Continue with Address"} onPress={() => navigation.navigate('AddressSignup', {email, phoneNumber, name, password})} textColor={"white"} backgroundColor={"#0070CA"} width={300} />
        </View>
        </SafeAreaView>
    );
