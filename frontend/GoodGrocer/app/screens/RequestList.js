...
function RequestList(props) {
    ...
    const cards = communityRequestData.map((request) => <TouchableOpacity style={styles.requestCard} key={request.id} ><RequestCard  name={request.name} storeName={request.storeName} numItems={request.numItems} imageUri={request.imageUri} onPress={()=>props.navigation.navigate("RequestDetail", {requestId: request.id, storeName: request.storeName})}/></TouchableOpacity>)
    return <SafeAreaView style={styles.container}>
    <View style={styles.content}>
      <Image source={require("../assets/logo.png")}/>
      <Text style={styles.titleText}>Requests in {props.route.params.communityName}</Text>
